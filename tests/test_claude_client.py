...
def test_propose_creative_uses_combined_max_tokens(monkeypatch):
    fake = _FakeClient(_FakeResp("===案1===\n【型】USP型\n## スライド1\n[キャッチ]\n- a"))
    monkeypatch.setattr(claude_client, "_get_client", lambda key: fake)
    out = claude_client.propose_creative("k", {"productName": "珪藻土バスマット"}, "遅い")
    assert "案1" in out.text
    assert out.truncated is False
    assert fake.messages.called_with["model"] == "claude-sonnet-4-6"
    assert fake.messages.called_with["max_tokens"] == 12000
    assert "珪藻土バスマット" in fake.messages.called_with["messages"][0]["content"]


def test_propose_creative_flags_truncation(monkeypatch):
    fake = _FakeClient(_FakeResp("途中で切れた", stop_reason="max_tokens"))
    monkeypatch.setattr(claude_client, "_get_client", lambda key: fake)
    out = claude_client.propose_creative("k", {"productName": "X"}, "")
    assert out.truncated is True
